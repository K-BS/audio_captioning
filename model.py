...
from encoder import ASTModel,init_layer
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, ntoken, ninp, nhead, nhid, nlayers, batch_size, dropout=0.5,pretrain_cnn=None,
        ...
        self.fc1 = nn.Linear(768, nhid, bias=True)
        self.dec_fc = nn.Linear(nhid, ntoken)
        self.batch_size = batch_size
        self.ntoken = ntoken
        #print(ntoken)
        #hello = torch.('audioset_0.4593.pth') #cccc
        #pretrained_mdl_path = 'audioset_16_16_0.4422.pth'
        
        self.encoder = ASTModel(input_fdim=128, input_tdim=1024)
        
        #self.encoder.load_state_dict(hello, strict=False)#aaaa
        ...
        '''if pretrain_cnn is not None:
            ...
            #oh = 'audioset_0.4593.pth'
            #device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            #hi = torch.load(oh, map_location=device)
            self.encoder.load_state_dict(dict_new)
            #self.encoder.load_state_dict(hi)
        if freeze_cnn:
            self.freeze_cnn()'''
