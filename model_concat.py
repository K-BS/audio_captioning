...
from encoder import Cnn10,Cnn10_2,init_layer
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, ntoken, ninp, nhead, nhid, nlayers, batch_size, dropout=0.5,pretrain_cnn=None,
        ...
        self.fc2 = nn.Linear(nhid + nhid, nhid, bias=True)
        self.dec_fc = nn.Linear(nhid, ntoken)
        self.batch_size = batch_size
        self.ntoken = ntoken
        self.encoder = Cnn10()
        self.encoder2 = Cnn10_2()
    ...
    def encode(self, src, input_mask=None):
        ...
        print(x.size())
        x2 = self.encoder2(src2)  # (batch_size, 512, T/16, mel_bins/16)
        #print("1", x.size())
        x2 = torch.mean(x2, dim=3)  # (batch_size, 512, T/16)
        #print("2", x.size())
        x2 = x2.permute(2, 0, 1)  # (T/16,batch_size,512)
        x2 = F.relu_(self.fc(x2))
